<template>
  ...
  <component :is="wordfamilywidget" v-bind="currentWordfamily"></component>
  <component :is="relatedwordwidget" v-bind="currentRelatedwords"></component>
...
import wordfamilywidget from '../EditorWidgets/wordfamilywidget.vue'
import relatedwordwidget from '../EditorWidgets/relatedwordwidget.vue'
export default {
  components: {wordfamily, relatedwords, wordfamilywidget, relatedwordwidget},
  data() {
  return  {
    ...
    relatedwords: null,
    wordfamilywidget: null,
    relatedwordwidget: null,
    showNewButton: false
          }
        },
  methods:{
    ...
    swapEditWordfamily(payload){
      this.currentWordfamily = {word_family: payload.word_family,
      entryurl:payload.entryurl}
      this.wordfamilywidget = wordfamilywidget
    },
    swapEditRelatedwords(payload){
      this.currentRelatedwords = {relatedwords: payload.related_words,
      entryurl:payload.entryurl}
      this.relatedwordwidget = relatedwordwidget
    }
  },
  mounted(){
    EventBus.$on('wordfamily', this.swapWordfamily)
    EventBus.$on('relatedwords', this.swapRelatedwords)
    EventBus.$on('editwordfamily', this.swapEditWordfamily)
    EventBus.$on('editrelatedwords', this.swapEditRelatedwords)
           }
}
</script>
