...
class Verb:
    ...
    def __init__(self, **kwargs):
        kwargs = {**self.DEFAULTDICT, **kwargs}
        self.subj = TOPERSON.get(kwargs['subj'])
        self.obj = TOPERSON.get(kwargs['obj'])
        order, mode, focus, type, lemma, polarity = (kwargs[k] for k in ('order', 'mode', 'focus', 'type', 'lemma', 'polarity'))
        self.order, self.mode, self.focus = (kwargs[k] for k in ('order', 'mode', 'focus'))
        if self.focus == 'actor':
    ...
    def conjugate(self):
        TYPEDICT = {
        'vai': [self.vai_select,{}],
        'vai2': [self.vai_select,{'am': True}],
        'vai + o': [self.vti_select,{'vai + o': True}],
        'vti2': [self.vti_select,{'oo':True}],
        'vti': [self.vti_select,{'am': True}],
        'vii': [self.vii_select,{}],
        'vta': [self.vta_select,{}],
        'vta-izhi': [self.vta_select,{'izhi': True}],
        }
        if self.order == 'changedconjunct':
            initial_vowel_change(self.morphemelist[0])
        calling = TYPEDICT[self.type]
        calling[0](**calling[1])
        self.assemble()
        return todoublevowel(self.form)
    def vai_select(self, **kwargs):
        self.verb_irregularities = kwargs
        if 'am' in kwargs:
    ...
    def vii_select(self, **kwargs):
        ...
        if self.subj.plurality and self.order == 'independent':
            if (not self.polarity) and (not self.subj.obviacy) and self.mode == 'neutral':
                ...
                self.morphemelist.append(An_())
    def vta_select(self, **kwargs):
        izhi = kwargs.get('izhi')
        self.verb_irregularities = kwargs
        if self.subj == p1p and self.obj in [p2, p2p]:
            self.subj= pX
        elif not self.subj.animacy:
            if izhi:
                self.morphemelist[-1].form = ['i']
