...
class Table(Tag):
    def __init__(self,  dim1list, dim2list, attributes = '', kwarglist = [], caption = None,  page = None, funcname = "Verb"):
        ...
        for i, nn in enumerate(dim1list):
            ...
            self.dim1widthlist.append(width[-1] if width.size>0 else 1)
        self.dim1repeatlist = []
        for i, nn in enumerate(dim1list):
            repeat = np.cumprod(self.dim1lenlist[i+1:])
            self.dim1repeatlist.append(repeat[-1] if repeat.size>0 else 1)
        ...
        for i, nn in enumerate(dim2list):
            ...
            self.dim2widthlist.append(width[-1] if width.size>0 else 1)
        self.dim2repeatlist = []
        for i, nn in enumerate(dim2list):
            repeat = np.cumprod(self.dim2lenlist[i+1:])
            self.dim2repeatlist.append(repeat[-1] if repeat.size>0 else 1)
...
class Page(Tag):
    def __init__(self,  layout, dim3list, kwarglist, funcname = 'Verb'):
        ...
        for pairs in it.product(*self.dim3arglist):
            ...
            self.append(f'<div id = "{self.title}" class = "tabcontent">{group}</div>')
    def __str__(self):
...
def vaioutput(lemma, type):
    body = str(Neutvai(Littlepage([f'lemma = "{lemma}"', f' type = "{type}"' ]))) + str(Impvai(Littlepage([f'lemma = "{lemma}"', f' type = "{type}"'])))
    return makepage(body)
def viioutput(lemma, type):
    body = str(Neutvii(Littlepage([f'lemma = "{lemma}"', f' type = "{type}"'])))
    return makepage(body)
def vtioutput(lemma, type):
    body = str(Neutvtiind(Littlepage([f'lemma = "{lemma}"', 'order = "independent"', f' type = "{type}"']))) +  \
    str(Neutvticonj(Littlepage([f'lemma = "{lemma}"', 'order = "conjunct"', f' type = "{type}"']))) + \
    str(Impvti(Littlepage([f'lemma = "{lemma}"', f' type = "{type}"'])))
    return makepage(body)
def vtaoutput(lemma, type):
    body = str(Page(kwarglist = [f'lemma = "{lemma}"', f' type = "{type}"'], dim3list = [biorders, regmodes, polarities], layout=vtaneutstruct)) + \
        str(Page(kwarglist = [f'lemma = "{lemma}"', f' type = "{type}"'], dim3list = [imporder], layout=vtaimpstruct))
    return makepage(body)
def naoutput(lemma, type):
    pass
def nioutput(lemma, type):
    pass
CALLDICT = {
    'vai':vaioutput,
    'vai2': vaioutput,
...
def conj_call(entry, inspect=False):
    """determines what function call to make to conjugate
    a given word based on its part of speech"""
    lemma = entry.stem[1:-2].translate(str.maketrans({"+":None}))   
    pos = entry.part_of_speech.abbrev
    if lemma == "iN": pos = "vta-izhi"
    retval = CALLDICT[pos](type=pos, lemma=lemma)
    if inspect:
